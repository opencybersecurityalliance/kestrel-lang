...
@pytest.mark.parametrize(
    "pattern",
    ["pid IN (1, 2, 3)", "[pid IN (1, 2, 3)]", "[process:pid IN (1, 2, 3)]"],
)
...
def test_quoted_datasource():
    results = parse_kestrel(
        "y = get url from \"udi://My QRadar\" where [url:value LIKE '%']"
    )
    result = results[0]
    assert result["command"] == "get"
    assert result["type"] == "url"
...
def test_apply_params_with_at():
    results = parse_kestrel(
        'apply xyz://my_analytic on foo with x="https://www.xyz.com/123@me.com/action"'
    )
    result = results[0]
    assert result["command"] == "apply"
    assert result["analytics_uri"] == "xyz://my_analytic"
...
def test_apply_params_with_decimal_and_dots():
    results = parse_kestrel(
        "apply xyz://my_analytic on foo with x=0.1, y=[a.value ,b,c]"
    )
    result = results[0]
    assert result["command"] == "apply"
    assert result["analytics_uri"] == "xyz://my_analytic"
...
def test_grouping_2():
    results = parse_kestrel(
        "y = group x BY foo, bar WITH MAX(baz) AS biggest, MIN(blah)"
    )
    result = results[0]
    print(result)
    assert result["command"] == "group"
